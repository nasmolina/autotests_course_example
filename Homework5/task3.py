...
def everything_for_your_cat(cats_data):
    ...
    for cat in cats_data:
        ...
        if (owner_name, owner_surname) in owners:
            owners[(owner_name, owner_surname)].append((name, age))
        else:
            owners[(owner_name, owner_surname)] = [(name, age)]
    our_str = ''
    for owner, cats in owners.items():
        cats_str = ''
        for cat in cats:
            cats_str += cat[0] + ', ' + str(cat[1]) + '; '
        cats_str = cats_str[:-2]  # удаляем последние два символа "; "
        our_str += owner[0] + ' ' + owner[1] + ': ' + cats_str + '\n'
    return our_str
